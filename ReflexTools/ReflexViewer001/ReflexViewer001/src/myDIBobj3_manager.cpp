...
*/
void MyDIBObj3 :: SaveBitmapFile( char *filename , int isfc , int x , int y , int width , int height )
{
	if(!SFCCheck(isfc))return;
...
int WIDTH ;
	WIDTH = (width+3)/4*4 ;
FILE *fp ;
	if( fopen_s( &fp , filename , "wb" ) )return ;
BYTE buf[256] ;
int  tmp ;
	buf[0] = 'B' ;
	buf[1] = 'M' ;
...
HFONT tmpfont;
	...
	if(tmpfont == NULL)
	{
		DEBUG_OUTPUT(_T("textdraw() フォントの作成に失敗！"));
		return;
	}
