...
int YumeDXDecode( unsigned char **PPout , unsigned char *Pin , int rlimit ,int *Prsize , int *Pwsize )
{
#define  READ(dest)			{if(rpos>=rlimit){free(Ptmp);return-1;}(dest)=Pin[rpos];}
unsigned char *Ptmp ;
